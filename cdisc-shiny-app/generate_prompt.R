...
generate_llm_prompt <- function(sdtm_data, target_adams, complexity) {
  ...
  prompt <- paste0(
    ...
    "          sources: [DM.STUDYID]\n",
    "- **Derivation Logic**:\n",
    "-   Use valid R `dplyr` syntax for `logic`.\n",
    "-   **Grouping**: If a derivation requires specific grouping (e.g., counters like `ASEQ`, `AFL`), specify `group_by` at the **variable level** (inside `derivation`).\n",
    "-   Example: `derivation: { logic: \"row_number()\", group_by: [\"USUBJID\"] }`\n",
